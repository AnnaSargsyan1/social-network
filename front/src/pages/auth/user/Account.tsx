...
export const Account = () => {
  ...
  useEffect(() => {
    ...
    //     setIsFollowing(response.data.payload.connection.following);
    //   })
    //   .catch((err) => {
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {account.posts.map((post) => (
                      <Link to={`/posts/${post.id}`} key={post.id}>
                        <div
                          className="relative w-full aspect-square bg-gray-800 rounded-lg overflow-hidden shadow-md group hover:shadow-xl transition-shadow cursor-pointer"
                        >
                          {/* Post Image */}
                          <Image
                            src={post.picture}
